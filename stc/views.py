...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
def logs_from_qr_id(request, username, qr_ids, depot_id):
    # workingSheetFromQRID method
    if request.method == 'GET':
        # Check if the requested username matches the logged-in user
        if request.user.username != username & request.user.depot_id == depot_id:
            return Response({'error': 'Unauthorized'}, status=status.HTTP_401_UNAUTHORIZED)
...
@authentication_classes([JWTAuthentication])
@permission_classes([IsAuthenticated])
def save_working_sheet_qr(request, username,depot_id,qr_ids):
    if request.method == 'GET':
        ...
        try:
            ...
            for qr_id in qr_id_list:
                try:
                    log = Master_Data.objects.get(qr_id=qr_id, depot_id=depot_id)
                    material_numbers.append(str(log.material_no))


                except Master_Data.DoesNotExist:
                    error_message = f'A log with QR ID [ {qr_id} ] at Depot [ {depot_id} ] does not exist.'
                    errors.append(error_message)
